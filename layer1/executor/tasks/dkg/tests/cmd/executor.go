...
func ReplaceConfigurationFile(workingDir, address, privateKey string, listeningPort, p2pPort, discoveryPort, localStatePort, index int) error {
	...
	result = regex.ReplaceAllString(result, "passcodes = \""+filepath.Join(workingDir, "keystores", "passcodes.txt")+"\"") // TODO - check file path root project or working dir
	regex = regexp.MustCompile(`keystore = .*`)
	result = regex.ReplaceAllString(result, "keystore = \""+filepath.Join(workingDir, "keystores", "keys")+"\"")
